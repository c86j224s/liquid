...
For last-attempt high/strict historical recovery work, prefer the isolated `historical_phase_engine` fixture gate before any further legacy-loop patch. Enable it only with `LIQUID_RESEARCH_HISTORICAL_PHASE_ENGINE=1`; default behavior remains legacy. The vertical slice is inserted before the per-iteration legacy loop in `execute_ai_task_with_quality_loop`, and on a matching historical request it saves directly through normal task completion. Its accepted output is rendered directly from engine-owned Source Cards, Claim Log, Phase Plan, and Phase Cards, and it must not use model-emitted artifact JSON, the legacy finalizer, the repair prompt loop, or the bounded work queue as the source of truth. Source support is strict: a phase claim needs matching extracted facts on its supporting Source Card, not only a plausible title or URL. If it fails, assign exactly one layer: `source_cards`, `claim_log`, `phase_plan`, `phase_card`, `renderer`, `acceptance`, or `integration`.

Within that isolated slice, acceptance now has four deterministic verdicts. `research_grade` means the engine built granular phase claims, claim-backed causal spines, and a non-template final answer with concrete phase/time references. `explainer` means the report is evidence-backed but still too broad or generic for strict historical acceptance. `partial` means some cards are trustworthy but other source/claim support remained open debt. `blocked` means the evidence ledger never reached the minimum trustworthy source/claim scaffold.
