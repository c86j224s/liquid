...
A bounded `narrative_enrichment` controller stage now runs after artifact parsing/persistence and before finalization. Its first enabled strategy is historical event-card enrichment. The stage selects only weak cards, calls the existing `ModelRuntime` with web search disabled and a compact JSON-only prompt, merges only fields grounded in existing Source Cards / Claim Log rows, and turns invalid JSON, invented IDs, or ungrounded detail into specific research debt. Non-historical modes should remain behavior-preserving until a future strategy is explicitly added.
