...
For high-intensity strict historical runs, hidden `narrative_state` or `reader_quality` artifacts are still diagnostic-only, not evidence, but they are now expected to carry useful planning depth. Engine-owned historical phase state may repair event-card identity from grounded phase scaffolding, but unsupported development/outcome prose must remain empty or open debt until later grounded enrichment. A historical run that keeps both empty should surface an explicit repair failure rather than pass as a shallow but clean-looking report.
