...
The controller now has a bounded narrative/artifact enrichment stage between artifact persistence and finalization. The first enabled strategy is strict/high historical event-card enrichment: weak `narrative_state.event_cards` are selected, a compact JSON-only model call may propose missing phase fields plus nested `causal_spine` and `interpretive_layers`, and only fields grounded in existing public Source Cards / supported Claim Log rows are merged. Unsupported or ungrounded proposals become `ResearchDebtItem`s; hidden narrative artifacts remain non-evidentiary. The internal enrichment prompt is transient and must not replace the task's stored resolved prompts. Non-historical modes are unchanged in this pass, but the app orchestration is intentionally shaped so future technology, policy, local recommendation, or comparative-decision enrichment strategies can plug into the same bounded stage.

Strict/high historical mode now inserts an engine-owned `phase_state` step before model-backed enrichment. The engine treats model-emitted event cards as candidate input rather than authoritative state: placeholder cards are rejected, broad process topics can be decomposed into phase candidates, and each phase must resolve to phase-specific supported Claim Log rows before it can become a grounded event card. Oversized but syntactically valid machine artifact JSON is parsed, normalized, compacted, and annotated instead of discarding Source Cards / Claim Log rows solely because the raw block exceeded the persistence budget; raw oversized JSON, prompts, diagnostics, and provider payloads remain local-only.


Experimental high/strict historical runs can now opt into an isolated `historical_phase_engine` vertical slice with `LIQUID_RESEARCH_HISTORICAL_PHASE_ENGINE=1`. This path is deliberately default-off and is inserted before the legacy per-iteration research loop. When the flag and historical request gate both match, the engine treats Source Cards and Claim Log rows as the evidence boundary, builds a phase plan and phase cards from engine-owned state, renders the final markdown and verification appendix itself, and saves directly through normal task completion without calling the legacy finalizer, the repair prompt loop, or the bounded work queue as the source of truth. Strict source support is semantic, not cosmetic: source title or URL alone never supports a phase claim, and supporting Source Cards must keep non-empty extracted facts whose content matches the admitted Claim Log row. Fixture failures should be classified into exactly one layer: `source_cards`, `claim_log`, `phase_plan`, `phase_card`, `renderer`, `acceptance`, or `integration`; live failures after fixture success are source/claim adapter failures unless accepted phase cards exist and only prose quality regresses.

The isolated engine now tracks a local verdict tier inside `quality_gate.status`: `research_grade`, `explainer`, `partial`, or `blocked`. Top-level task `quality_status` remains compatibility-oriented (`passed`, `untrusted`, `blocked`). `research_grade` requires granular claim types, claim-backed causal spine coverage on the phase cards, and a non-template final answer that actually names enough phase labels and time anchors. `explainer` means the evidence is real but still too broad or too flat for strict historical acceptance. `partial` means some usable evidence survived while other claim/source support stayed open debt. `blocked` means the engine could not build the minimum trustworthy source/claim scaffold.

The engine still emits a compact `[RESEARCH_ARTIFACT_JSON]` appendix block and persists controller artifacts for replay, but both surfaces are sanitized checkpoints only. They may include compact Source Cards, Claim Log rows, phase cards, quality-gate state, and bounded warnings/debt. They must not contain raw prompts, provider payloads, raw model output, or raw diagnostics.

That stage is now driven by a bounded resumable research work queue persisted inside the existing controller-artifact envelope. The queue stores only typed work items, mapped debt IDs, bounded dependencies, layered wave/model-call/work-item/attempt budgets, conservative terminal status, compact checkpoints, and terse progress summaries. Every declared work-item kind now has a routed bounded executor path: artifact stabilization re-runs safe parse/compact/salvage, source/claim repair uses only existing deterministic public-evidence repair paths or blocks with explicit next action, phase and causal review run deterministic readiness checks, final-answer render invokes the artifact-backed finalizer, and acceptance review invokes the existing quality gates. Work items may reference phase IDs, Claim Log IDs, Source Card IDs, and sanitized fingerprints, but the queue still does not store raw prompts, provider payloads, raw model output, or raw diagnostics.
