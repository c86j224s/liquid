...
impl CliLaunchMode {
    pub(crate) fn resolved(self) -> ResolvedCliLauncher {
        match self {
    ...
    pub(crate) fn can_launch_cli(self) -> bool {
...
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub(crate) enum ResolvedCliLauncher {
    SandboxExec,
    Unsandboxed,
    Disabled,
}
...
pub(crate) fn launcher_unavailable_message(mode: CliLaunchMode) -> String {
    format!(
...
impl CliInvocation {
    pub(crate) fn new(program: impl Into<String>, current_dir: impl AsRef<Path>) -> Self {
        Self {
...
pub(crate) fn generic_sandbox_profile(home: &str, data_dir: &str) -> String {
    format!(
        r#"(version 1) 
           (allow default) 
           (deny file-write* 
...
pub(crate) fn pi_sandbox_profile(pi_home: &str, agent_dir: &str, session_dir: &str) -> String {
    format!(
        r#"(version 1)
                   (deny default)
