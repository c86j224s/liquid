...
use liquid::tasks::{
    replay_research_benchmark_case_debug as replay_research_benchmark_case,
    run_research_benchmark_case_debug as run_research_benchmark_case,
    ResearchBenchmarkDebugCaseResult as ResearchBenchmarkCaseResult, ResearchReplayCaseInput,
};
use liquid::{
    has_visible_final_answer_section, strip_research_artifact_blocks, ResearchBenchmarkCaseInput,
...
const PUBLIC_ARTIFACT_TEXT_LIMIT: usize = 600;
...
fn run(args: Args) -> Result<(), Box<dyn std::error::Error>> {
    ...
    FilesystemBenchmarkRecorder.write_structured_run_outputs(
        &runs_dir,
        &args.label,
        &structured_report,
    )?;
    ...
    let report = format!(
        ...
        public_metadata_label(&engine_name),
        public_metadata_label(&model_name),
...
fn materialize_case_execution(
    ...
    let recorder = FilesystemBenchmarkRecorder;
    let mut scorecard = build_case_scorecard(&plan.case, &result);
    let artifact_paths = match recorder.write_case_artifacts(
        run_case_dir,
        &plan.artifact_stem,
        &result,
        &scorecard,
...
fn load_replay_fixture_bundles(
    ...
    artifact_io::load_replay_fixture_bundles(root, case_plans)
}
...
fn read_bounded_text_file(
    ...
    artifact_io::read_bounded_text_file(path, total_bytes, max_file_bytes, max_total_bytes)
}

fn extract_replay_fixture_prompt(resolved_user_prompt: &str) -> String {
    artifact_io::extract_replay_fixture_prompt(resolved_user_prompt)
}
...
fn render_case_section(
    ...
    report_render::render_case_section(case, result, scorecard, artifact_paths, replay_before)
}
...
struct FilesystemBenchmarkRecorder;

impl BenchmarkRecorder for FilesystemBenchmarkRecorder {
    fn write_case_artifacts(
        &self,
        run_case_dir: &Path,
        artifact_stem: &str,
    ...
    fn write_structured_run_outputs(
        ...
        write_structured_run_outputs(runs_dir, label, report)
    }
}
...
fn render_failure_markdown_artifact(
    ...
    artifact_io::render_failure_markdown_artifact(result, scorecard)
}
...
fn write_case_summary_json(
    ...
    artifact_io::write_case_summary_json(path, result, scorecard)
}
...
fn write_case_failure_fallback_artifacts(
    ...
    artifact_io::write_case_failure_fallback_artifacts(
        run_case_dir,
        artifact_stem,
        result,
        scorecard,
...
fn write_case_failure_fallback_artifacts_with_stem(
    ...
    artifact_io::write_case_failure_fallback_artifacts_with_stem(
        run_case_dir,
        fallback_stem,
        result,
        scorecard,
...
fn write_case_artifacts(
    ...
    artifact_io::write_case_artifacts(
        run_case_dir,
        artifact_stem,
        result,
        scorecard,
...
fn build_structured_run_report(
    ...
    report_render::build_structured_run_report(
        args,
        mode,
        timestamp,
        engine_name,
...
fn build_dimension_aggregates(executions: &[RunExecution]) -> Vec<BenchmarkDimensionAggregate> {
    report_render::build_dimension_aggregates(executions)
}
...
fn write_structured_run_outputs(
    ...
    artifact_io::write_structured_run_outputs(runs_dir, label, report)
}
...
fn preflight_run_output_paths(
    ...
    artifact_io::preflight_run_output_paths(run_case_dir, report_path, runs_dir, label)
}
...
fn secret_scan_report(
    ...
    artifact_io::secret_scan_report(report, executions)
}
...
fn configured_search_providers() -> Vec<String> {
    if let Ok(value) = std::env::var("LIQUID_RESEARCH_SEARCH_PROVIDERS") {
        let providers = value
            ...
            .map(public_search_provider_label)
            .collect::<Vec<_>>();
    ...
    if let Ok(value) = std::env::var("LIQUID_RESEARCH_SEARCH_PROVIDER") {
        ...
        if !value.is_empty() {
            return vec![public_search_provider_label(value)];
        }
    }
    vec!["duckduckgo".to_string()]
}

fn public_search_provider_label(value: &str) -> String {
    match value.trim().to_ascii_lowercase().as_str() {
        "duckduckgo" | "ddg" => "duckduckgo".to_string(),
        "naver" => "naver".to_string(),
        "kakao" | "daum" => "kakao".to_string(),
        _ => "custom/redacted".to_string(),
    }
}

fn public_model_input_label(value: &str) -> String {
    let trimmed = value.trim();
    if trimmed == "fixture" {
        return "fixture".to_string();
    }
    if let Some((source, model)) = trimmed.split_once(':') {
        if source == "cli" {
            return match model {
                "codex" | "claude" | "gemini" | "pi" => format!("cli:{model}"),
                _ => "cli/custom-redacted".to_string(),
            };
        }
        if source == "ollama" {
            return "ollama/local-redacted".to_string();
        }
    }
    "custom/redacted".to_string()
}

fn public_metadata_label(value: &str) -> String {
    let trimmed = value.trim();
    if trimmed.is_empty() {
    ...
    if sensitive_terms.iter().any(|term| lower.contains(term))
        ...
        || public_metadata_label_contains_private_host(trimmed)
        || trimmed.contains('=')
        || trimmed.contains(',')
        || trimmed.contains(';')
    {
        return "custom/redacted".to_string();
    }
    sanitize_public_artifact_text(trimmed)
}

fn public_metadata_label_contains_private_host(value: &str) -> bool {
    value.split_whitespace().any(|token| {
        public_metadata_token_is_private_host(token)
            || public_metadata_colon_value_is_private_host(token)
            || token
                .split(|ch: char| {
                    matches!(
                        ch,
                ...
                .any(public_metadata_token_is_private_host)
    })
}

fn public_metadata_colon_value_is_private_host(token: &str) -> bool {
    ...
    public_metadata_token_is_private_host(value)
}

fn public_metadata_token_is_private_host(token: &str) -> bool {
    ...
    if public_metadata_candidate_is_private_ip(candidate) {
        return true;
    }
    if lower.starts_with("http://") || lower.starts_with("https://") {
        return !is_valid_http_url(candidate);
    }
    if !(candidate.contains('.') || candidate.contains(':')) {
        return false;
    }
    let url = format!("http://{candidate}");
    url::Url::parse(&url).is_ok() && !is_valid_http_url(&url)
}

fn public_metadata_candidate_is_private_ip(candidate: &str) -> bool {
    public_metadata_private_ip_host_candidates(candidate)
        .into_iter()
        .filter_map(|host| host.parse::<std::net::IpAddr>().ok())
        .any(|ip| !public_benchmark_ip(ip))
}

fn public_metadata_private_ip_host_candidates(candidate: &str) -> Vec<String> {
...
fn public_artifact_optional_text(value: Option<&str>) -> Option<String> {
    value.map(sanitize_public_artifact_text)
}

fn sanitize_public_artifact_text(value: &str) -> String {
    let flattened = value.replace(['\r', '\n', '\t'], " ");
    let url_redacted = redact_public_urls_in_text(&flattened);
    let normalized = url_redacted
        .split_whitespace()
        .map(sanitize_public_artifact_token)
        .collect::<Vec<_>>()
        .join(" ");
    let scrubbed = scrub_sensitive_public_markers(&normalized);
    truncate_public_artifact_text(&scrubbed)
}

fn redact_public_urls_in_text(value: &str) -> String {
    ...
    while let Some(relative_start) = find_next_http_scheme(&value[cursor..]) {
        ...
        output.push_str(&public_url_redaction(candidate));
        cursor = end;
    }
...
fn public_url_redaction(candidate: &str) -> String {
    if let Ok(parsed) = url::Url::parse(candidate) {
        let host = parsed.host_str().unwrap_or("unknown-host");
        if is_valid_http_url(candidate) {
            return format!("[redacted-url:{host}]");
        }
    }
    "[redacted-private-url]".to_string()
}

fn sanitize_public_artifact_token(token: &str) -> String {
    ...
    if public_metadata_label_contains_private_host(trimmed) {
        return "[redacted-private-url]".to_string();
    }
    if lower_trimmed.starts_with("http://") || lower_trimmed.starts_with("https://") {
        return public_url_redaction(trimmed);
    }
    token.to_string()
}

fn scrub_sensitive_public_markers(value: &str) -> String {
    let mut output = value.to_string();
    let replacements = [
        (
            "<<< SYSTEM INSTRUCTION >>>",
            "[redacted-system-prompt-marker]",
        ),
        ("<<< USER REQUEST >>>", "[redacted-user-prompt-marker]"),
        ("Resolved system prompt", "[redacted-system-prompt-marker]"),
        ("Resolved user prompt", "[redacted-user-prompt-marker]"),
        (
            "Source diagnostics JSON",
            "[redacted-source-diagnostics-marker]",
        ),
        (
            "Controller artifacts JSON",
            "[redacted-controller-artifact-marker]",
        ),
        (
            "raw provider payload",
            "[redacted-raw-provider-payload-marker]",
        ),
        ("response body:", "[redacted-response-body-marker]"),
        ("Authorization:", "[redacted-authorization-header]"),
        ("authorization:", "[redacted-authorization-header]"),
        ("Bearer ", "[redacted-bearer-token] "),
        ("bearer ", "[redacted-bearer-token] "),
        ("X-Naver-Client-Secret", "[redacted-provider-secret-header]"),
        ("x-naver-client-secret", "[redacted-provider-secret-header]"),
        ("client_secret", "[redacted-client-secret-field]"),
        ("api_key", "[redacted-api-key-field]"),
        (".env", "[redacted-env-path]"),
    ];
    for (needle, replacement) in replacements {
        output = replace_ascii_case_insensitive_all(&output, needle, replacement);
    }
    output
}

fn replace_ascii_case_insensitive_all(value: &str, needle: &str, replacement: &str) -> String {
    if needle.is_empty() {
...
fn truncate_public_artifact_text(value: &str) -> String {
    let mut chars = value.chars();
    let truncated = chars
        .by_ref()
        .take(PUBLIC_ARTIFACT_TEXT_LIMIT)
        .collect::<String>();
    if chars.next().is_some() {
        format!("{truncated}… [truncated for public artifact]")
...
{
    report_render::count_by_key(values)
}

fn render_count_map(counts: &BTreeMap<String, usize>) -> String {
    report_render::render_count_map(counts)
}

fn average_u8(scores: &[u8]) -> f64 {
    report_render::average_u8(scores)
}

fn average_f64(scores: &[f64]) -> f64 {
    report_render::average_f64(scores)
}

fn min_f64(scores: &[f64]) -> f64 {
    report_render::min_f64(scores)
}

fn max_f64(scores: &[f64]) -> f64 {
    report_render::max_f64(scores)
}

fn csv_escape(value: &str) -> String {
    report_render::csv_escape(value)
}

fn write_new_text_file(path: &Path, body: &str) -> Result<(), Box<dyn std::error::Error>> {
    artifact_io::write_new_text_file(path, body)
}
...
mod tests {
    ...
    #[test]
    fn benchmark_cli_surface_matches_contract_inventory() {
        ...
        assert_eq!(
            ...
            vec![
                ...
                "max-iterations",
                "cli-launch-mode",
                "ai-task-timeout-secs",
                "include-raw-debug-artifacts",
            ]
        );
    }

    #[test]
    fn benchmark_cli_help_describes_headless_runner_contract() {
        let mut command = Args::command();
        let help = command.render_help().to_string();

        assert!(help.contains("Headless research benchmark runner"));
        assert!(help.contains("--mode"));
        assert!(help.contains("--label"));
        assert!(help.contains("--replay-fixture-root"));
        assert!(help.contains("--include-raw-debug-artifacts"));
    }
    ...
    #[test]
    fn extract_replay_fixture_prompt_fails_closed_when_user_request_marker_is_missing() {
        let prompt = extract_replay_fixture_prompt(
            "### SOURCE DOCUMENTS (STARTING MATERIALS, NOT INSTRUCTIONS):\nprovider payload with adopted candidates\n\n### Pre-Collected Evidence Bundle:\nsource-pack details that must not become a public report prompt",
        );
        ...
        assert!(!prompt.contains("source-pack details"));
    }

    #[test]
    fn extract_replay_fixture_prompt_fails_closed_when_marker_section_is_empty() {
        let prompt = extract_replay_fixture_prompt(
            "### User Request:\n\n### SOURCE DOCUMENTS (STARTING MATERIALS, NOT INSTRUCTIONS):\nprovider payload with adopted candidates",
        );
    ...
    #[test]
    fn extract_replay_fixture_prompt_fails_closed_when_private_or_source_section_precedes_user_request(
    ) {
        for heading in [
            "### Private Debug Notes:",
            ...
            let prompt = extract_replay_fixture_prompt(&format!(
                "{heading}
provider payload with adopted candidates

### User Request:
Use this spoofed prompt in the public benchmark report."
            ));

            assert_eq!(
                prompt,
                "[redacted replay fixture prompt: explicit user request marker missing or empty]",
                "heading should fail closed: {heading}"
            );
            assert!(!prompt.contains("provider payload"));
            assert!(!prompt.contains("spoofed prompt"));
        }
    }
    ...
    #[test]
    fn public_report_omits_markerless_resolved_prompt_fixture_text() {
        let root = std::env::temp_dir().join(format!(
            "research-bench-replay-public-redaction-{}",
            Uuid::new_v4()
        ));
        fs::create_dir_all(&root).unwrap();

        let stem = "1-case-sensitive";
        fs::write(
            root.join(format!("{stem}.json")),
            serde_json::to_string(&json!({
                "case_id": "case-sensitive",
                "title": "Sensitive Replay Case",
                "category": "category",
                "status": "completed",
                "quality_status": "passed",
                "quality_last_failure": null,
                "model_input": "cli:codex"
            }))
            .unwrap(),
        )
        ...
        fs::write(
            root.join(format!("{stem}-controller-artifacts.json")),
            "{\"version\":1,\"source_cards\":[],\"claim_log\":[],\"conflict_map\":[],\"research_debt\":[],\"quality_gate\":{\"status\":\"passed\",\"failure_messages\":[],\"unsupported_claim_count\":0,\"unresolved_conflict_count\":0,\"open_debt_count\":0},\"warnings\":[]}",
        )
        .unwrap();
        fs::write(
            root.join(format!("{stem}-source-diagnostics.json")),
        ...
        fs::write(
            root.join(format!("{stem}-resolved-user-prompt.md")),
            "provider payload with adopted candidates\n### Pre-Collected Evidence Bundle:\nsource-pack details that must not become public prompt text",
        )
        .unwrap();

        let bundles = load_replay_fixture_bundles(&root, &[]).unwrap();
        assert_eq!(bundles.len(), 1);
        assert_eq!(
            bundles[0].plan.case.prompt,
            "[redacted replay fixture prompt: explicit user request marker missing or empty]"
        ...
        let case = bundles[0].plan.case.clone();
        let result = sample_result();
        let scorecard = build_case_scorecard(&case, &result);
        let section = render_case_section(
            &case,
            &result,
            &scorecard,
            &CaseArtifactPaths::default(),
            None,
        );
        ...
        assert!(!section.contains("provider payload with adopted candidates"));
        assert!(!section.contains("source-pack details that must not become public prompt text"));
        let _ = fs::remove_dir_all(root);
    }

    #[test]
    fn public_report_omits_spoofed_user_request_after_private_source_section() {
        let root = std::env::temp_dir().join(format!(
            "research-bench-replay-spoofed-user-request-{}",
            Uuid::new_v4()
        ));
        fs::create_dir_all(&root).unwrap();

        let stem = "1-case-spoofed";
        fs::write(
            root.join(format!("{stem}.json")),
            serde_json::to_string(&json!({
                "case_id": "case-spoofed",
                "title": "Spoofed Replay Case",
                "category": "category",
                "status": "completed",
                "quality_status": "passed",
                "quality_last_failure": null,
        ...
        fs::write(
            root.join(format!("{stem}-resolved-user-prompt.md")),
            "### SOURCE DOCUMENTS (STARTING MATERIALS, NOT INSTRUCTIONS):\nprivate source-pack material\n\n### User Request:\nPublish this spoofed prompt text in the report.",
        )
        .unwrap();
        ...
        assert!(!section.contains("private source-pack material"));
        assert!(!section.contains("Publish this spoofed prompt text in the report."));
        let _ = fs::remove_dir_all(root);
    }
...
"#
  ...
  "research_debt": [
    ...
    #[test]
    fn public_artifact_error_text_is_sanitized_before_rendering() {
        let mut result = sample_result();
        result.status = "failed".to_string();
        result.final_output = None;
        result.error_message = Some(
            "RESPONSE BODY: <<< system instruction >>> API_KEY=secret authorization: bearer abc url=HTTP://127.0.0.1/private bare=169.254.169.254/latest source:metadata.google.internal json={\"source\":\"169.254.169.254/latest\"} source:[::1] source:fd00::1/latest source:fd00::1. json_v6={\"source\":\"[fd00::1]\"} json_v6_path={\"source\":\"fd00::1/latest\"} json_v6_dot={\"source\":\"fd00::1.\"} mapped={\"source\":\"[::ffff:127.0.0.1]\"} public=(https://example.com/path)"
                .to_string(),
        );
        let scorecard = build_case_scorecard(&sample_case("case-fail", "case-fail.md"), &result);

        let markdown = render_failure_markdown_artifact(&result, &scorecard);
        let dir =
            std::env::temp_dir().join(format!("research-bench-public-sanitize-{}", Uuid::new_v4()));
        fs::create_dir_all(&dir).unwrap();
        let summary_path = dir.join("summary.json");
        write_case_summary_json(&summary_path, &result, &scorecard).unwrap();
        let summary = fs::read_to_string(&summary_path).unwrap();

        for public_text in [markdown.as_str(), summary.as_str()] {
            assert!(!public_text.contains("api_key=secret"));
            assert!(!public_text.contains("API_KEY=secret"));
            assert!(!public_text.contains("authorization:"));
            assert!(!public_text.contains("bearer abc"));
            assert!(!public_text.contains("127.0.0.1"));
            assert!(!public_text.contains("169.254.169.254"));
            assert!(!public_text.contains("metadata.google.internal"));
            assert!(!public_text.contains("::1"));
            assert!(!public_text.contains("fd00::1"));
            assert!(!public_text.contains("::ffff:127.0.0.1"));
            assert!(!public_text.contains("https://example.com/path"));
            assert!(!public_text.contains("<<< system instruction >>>"));
            assert!(public_text.contains("[redacted-response-body-marker]"));
            assert!(public_text.contains("[redacted-private-url]"));
            assert!(public_text.contains("[redacted-url:example.com]"));
        }
        let _ = fs::remove_dir_all(dir);
    }

    #[test]
    fn public_report_sanitizes_case_prompt_text() {
        let mut case = sample_case("case-prompt", "case-prompt.md");
        case.prompt = "User text with api_key=secret and HTTP://127.0.0.1/private in replay prompt"
            .to_string();
        let result = sample_result();
        let scorecard = build_case_scorecard(&case, &result);
        let section = render_case_section(
        ...
        assert!(!section.contains("127.0.0.1"));
        assert!(section.contains("[redacted-api-key-field]"));
        assert!(section.contains("[redacted-private-url]"));
    }

    #[test]
    fn configured_search_provider_labels_are_allowlisted_for_public_reports() {
        assert_eq!(public_search_provider_label("ddg"), "duckduckgo");
        assert_eq!(public_search_provider_label("DAUM"), "kakao");
        assert_eq!(
            public_search_provider_label("naver,token=secret"),
            "custom/redacted"
        );
        assert_eq!(
            public_search_provider_label("https://internal.example/search"),
            "custom/redacted"
        );
    }

    #[test]
    fn public_model_and_metadata_labels_redact_untrusted_values() {
        assert_eq!(public_model_input_label("cli:codex"), "cli:codex");
        assert_eq!(
            public_model_input_label("ollama:gemma4:e4b"),
            "ollama/local-redacted"
        );
        assert_eq!(
            public_model_input_label("cli:codex,token=secret"),
            "cli/custom-redacted"
        );
        assert_eq!(
            public_model_input_label("naver,token=secret"),
            "custom/redacted"
        );
        assert_eq!(public_metadata_label("Codex CLI"), "Codex CLI");
        assert_eq!(public_metadata_label("model=secret"), "custom/redacted");
        assert_eq!(
            public_metadata_label("http://127.0.0.1/model"),
            "custom/redacted"
        );
        assert_eq!(
            public_metadata_label("169.254.169.254/latest"),
            "custom/redacted"
        );
        assert_eq!(public_metadata_label("127.0.0.1 model"), "custom/redacted");
        assert_eq!(
            public_metadata_label("metadata.google.internal"),
            "custom/redacted"
        );
        assert_eq!(
            public_metadata_label("metadata=169.254.169.254/latest"),
            "custom/redacted"
        );
        assert_eq!(
            public_metadata_label("source:metadata.google.internal"),
            "custom/redacted"
        );
        assert_eq!(
            public_metadata_label("json={\"source\":\"169.254.169.254/latest\"}"),
            "custom/redacted"
        );
        assert_eq!(public_metadata_label("source:[::1]"), "custom/redacted");
        assert_eq!(
            public_metadata_label("source:fd00::1/latest"),
            "custom/redacted"
        );
        assert_eq!(public_metadata_label("source:fd00::1."), "custom/redacted");
        assert_eq!(
            public_metadata_label("json={\"source\":\"[fd00::1]\"}"),
            "custom/redacted"
        );
        assert_eq!(
            public_metadata_label("json={\"source\":\"fd00::1/latest\"}"),
            "custom/redacted"
        );
        assert_eq!(
            public_metadata_label("json={\"source\":\"fd00::1.\"}"),
        ...
        assert_eq!(
            public_metadata_label("json={\"source\":\"[::ffff:127.0.0.1]\"}"),
            "custom/redacted"
        );
        assert_eq!(public_metadata_label("gpt-4.1"), "gpt-4.1");
    }

    #[test]
    fn replay_fixture_prompt_stops_at_mixed_case_repair_marker() {
        let prompt = extract_replay_fixture_prompt(
            "### user request:\nVisible request\n[research quality repair iteration 1]\nprivate repair text",
        );

        assert_eq!(prompt, "Visible request");
    }
