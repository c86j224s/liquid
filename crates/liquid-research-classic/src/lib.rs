...
use liquid_research_core::{
    build_research_context_pack as core_build_research_context_pack,
    raw_fallback_context_diagnostics as core_raw_fallback_context_diagnostics,
    ResearchContextPackConfig,
};
...
pub const CLASSIC_RESEARCH_IMPLEMENTATION_ID: &str = "classic";

#[derive(Debug, Clone, Copy, Default)]
pub struct ClassicResearchImplementation;

#[derive(Debug, Clone, Copy)]
pub struct ClassicResearchContextPackOptions<'a> {
...
pub fn research_allows_web_search(file_prefix: &str) -> bool {
    matches!(file_prefix, "[Research]" | "[AI-Research]")
}

pub fn web_search_provider_for(source: &str, model_name: &str, requested: bool) -> &'static str {
    if !requested {
        return "none";
    }
...
pub fn web_search_audit_prompt(requested: bool, provider: &str) -> String {
    if !requested {
...
pub fn intensity_prompt(intensity: &str) -> &'static str {
    match intensity {
        "low" => {
...
pub fn research_controller_contract_prompt(format: &str) -> String {
...
pub fn fallback_disclosure_prompt(reason: Option<&str>) -> String {
    format!(
...
pub fn normalize_research_mode(mode: &str) -> &'static str {
...
pub fn normalize_research_type(research_type: Option<&str>, default_type: &str) -> &'static str {
    match research_type {
...
pub fn research_mode_lens(mode: &str) -> &'static str {
...
pub fn build_research_system_prompt(
    mode: &str,
    format: &str,
...
[Mode Lens]\n{}",
    ...
    if let Some(prompt) = html_design_prompt.filter(|_| format == "html") {
...
pub fn research_focus_section(instructions: Option<&str>) -> String {
...
pub fn build_document_research_user_prompt(instructions: Option<&str>) -> String {
    format!(
...
pub fn build_follow_up_research_user_prompt(instructions: Option<&str>) -> String {
    format!(
...
pub fn build_topic_research_user_prompt(topic: &str, instructions: Option<&str>) -> String {
    format!(
...
pub fn build_research_context_pack(
    ...
    options: ClassicResearchContextPackOptions<'_>,
) -> (String, ResearchContextPackingDiagnostics) {
    core_build_research_context_pack(
        user_prompt,
        raw_documents,
        artifacts,
        diagnostics,
        ResearchContextPackConfig {
            source_card_limit: options.source_card_limit,
            excerpt_chars: options.excerpt_chars,
            prompt_safe_ledger_text_chars: options.prompt_safe_ledger_text_chars,
            prompt_safe_ledger_list_items: options.prompt_safe_ledger_list_items,
            historical_narrative_state_prompt_hint: options.historical_narrative_state_prompt_hint,
            historical_reader_quality_prompt_hint: options.historical_reader_quality_prompt_hint,
        },
    )
}

pub fn raw_fallback_context_diagnostics(raw_documents: &str) -> ResearchContextPackingDiagnostics {
    core_raw_fallback_context_diagnostics(raw_documents)
}
...
mod tests {
    ...
    #[test]
    fn classic_research_implementation_id_is_frozen() {
        assert_eq!(CLASSIC_RESEARCH_IMPLEMENTATION_ID, "classic");
    }

    #[test]
    fn build_research_system_prompt_includes_supplied_html_design_prompt() {
        let prompt = build_research_system_prompt("technology", "html", Some("[HTML DESIGN TEST]"));

        assert!(prompt.contains("[HTML DESIGN TEST]"));
